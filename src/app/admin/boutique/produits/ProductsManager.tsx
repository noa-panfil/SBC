...
type ProductForm = {
    ...
    personalizationTextPrice: string;
    personalizationNumberPrice: string;
...
export default function ProductsManager() {
    ...
    useEffect(() => {
        setForm(selected ? {
            ...
            personalizationTextPrice: (selected.personalizationTextPriceCents / 100).toFixed(2),
            personalizationNumberPrice: (selected.personalizationNumberPriceCents / 100).toFixed(2),
    ...
    const saveProduct = async (event: FormEvent) => {
        ...
        const personalizationTextPrice = Number(form.personalizationTextPrice.replace(",", "."));
        const personalizationNumberPrice = Number(form.personalizationNumberPrice.replace(",", "."));
        if (form.personalizationEnabled && form.personalizationTextEnabled && (form.personalizationTextPrice.trim() === "" || !Number.isFinite(personalizationTextPrice) || personalizationTextPrice < 0)) {
            setMessage("Renseignez un tarif valide pour la personnalisation texte (0 € est autorisé).");
            return;
        }
        if (form.personalizationEnabled && form.personalizationNumberEnabled && (form.personalizationNumberPrice.trim() === "" || !Number.isFinite(personalizationNumberPrice) || personalizationNumberPrice < 0)) {
            setMessage("Renseignez un tarif valide pour la personnalisation numéro (0 € est autorisé).");
            return;
        }
        ...
        try {
            const data = await jsonRequest(selected ? `/api/admin/shop/products/${selected.id}` : "/api/admin/shop/products", {
                ...
                body: JSON.stringify({
                    ...
                    personalizationTextPriceCents: form.personalizationEnabled && form.personalizationTextEnabled ? Math.round(personalizationTextPrice * 100) : 0,
                    personalizationNumberPriceCents: form.personalizationEnabled && form.personalizationNumberEnabled ? Math.round(personalizationNumberPrice * 100) : 0,
                }),
            });
            ...
            <div className="mx-auto w-full max-w-[1500px] p-4 md:p-8">
                ...
                <div className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
                    ...
                    <div className="min-w-0">
                        ...
                        {activeTab === "details" && <section className={`mt-4 rounded-3xl border p-5 shadow-sm md:p-7 ${form.personalizationEnabled ? "border-sbc/30 bg-green-50" : "border-gray-200 bg-white"}`}><label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={form.personalizationEnabled} onChange={(event) => setForm({ ...form, personalizationEnabled: event.target.checked })} className="mt-1 h-5 w-5 accent-sbc" /><span><strong className="block text-base text-gray-950">Ce vêtement est personnalisable</strong><span className="mt-1 block text-sm font-normal leading-6 text-gray-600">Le client pourra ajouter un texte ou un numéro, devant ou dans le dos. Chaque option possède son propre tarif.</span></span></label></section>}
                        {activeTab === "details" && form.personalizationEnabled && <section className="mt-4 rounded-3xl border border-gray-200 bg-white p-5 shadow-sm md:p-7">
                            <div><h2 className="text-lg font-black text-gray-950">Options proposées au client</h2><p className="mt-1 text-sm text-gray-500">Activez chaque type, définissez son tarif puis choisissez où il peut être placé.</p></div>
                            <div className="mt-5 grid gap-4 sm:grid-cols-2">
                                <fieldset className={`rounded-2xl border p-4 ${form.personalizationTextEnabled ? "border-green-200 bg-green-50" : "border-gray-200 bg-gray-50"}`}>
                                    <legend className="px-1 text-sm font-black text-gray-950"><label className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={form.personalizationTextEnabled} onChange={(event) => setForm({ ...form, personalizationTextEnabled: event.target.checked })} className="h-4 w-4 accent-sbc" /><i className="fas fa-font text-sbc" />Texte</label></legend>
                                    {form.personalizationTextEnabled ? <div className="mt-3">
                                        <label className="block text-sm font-black text-gray-950">Tarif du texte (€)<input required min="0" step="0.01" inputMode="decimal" value={form.personalizationTextPrice} onChange={(event) => setForm({ ...form, personalizationTextPrice: event.target.value })} placeholder="Ex. 5,00" className="mt-2 w-full rounded-xl border border-gray-300 bg-white px-4 py-3 font-normal focus:border-sbc focus:outline-none focus:ring-4 focus:ring-sbc/15" /><span className="mt-1 block text-xs font-normal text-gray-500">Saisissez 0 pour rendre cette option gratuite.</span></label>
                                        <p className="mt-4 text-xs font-black uppercase tracking-wide text-gray-600">Le texte peut être placé</p><div className="mt-2 grid grid-cols-2 gap-2"><label className={`cursor-pointer rounded-xl border p-3 text-center text-sm font-bold ${form.personalizationTextFrontEnabled ? "border-sbc bg-sbc text-white" : "border-gray-200 bg-white text-gray-600"}`}><input type="checkbox" checked={form.personalizationTextFrontEnabled} onChange={(event) => setForm({ ...form, personalizationTextFrontEnabled: event.target.checked })} className="sr-only" />Devant</label><label className={`cursor-pointer rounded-xl border p-3 text-center text-sm font-bold ${form.personalizationTextBackEnabled ? "border-sbc bg-sbc text-white" : "border-gray-200 bg-white text-gray-600"}`}><input type="checkbox" checked={form.personalizationTextBackEnabled} onChange={(event) => setForm({ ...form, personalizationTextBackEnabled: event.target.checked })} className="sr-only" />Dos</label></div>
                                    </div> : <p className="mt-3 text-xs text-gray-500">Le texte ne sera pas proposé.</p>}
                                </fieldset>
                                <fieldset className={`rounded-2xl border p-4 ${form.personalizationNumberEnabled ? "border-green-200 bg-green-50" : "border-gray-200 bg-gray-50"}`}>
                                    <legend className="px-1 text-sm font-black text-gray-950"><label className="flex cursor-pointer items-center gap-2"><input type="checkbox" checked={form.personalizationNumberEnabled} onChange={(event) => setForm({ ...form, personalizationNumberEnabled: event.target.checked })} className="h-4 w-4 accent-sbc" /><i className="fas fa-hashtag text-sbc" />Numéro</label></legend>
                                    {form.personalizationNumberEnabled ? <div className="mt-3">
                                        <label className="block text-sm font-black text-gray-950">Tarif du numéro (€)<input required min="0" step="0.01" inputMode="decimal" value={form.personalizationNumberPrice} onChange={(event) => setForm({ ...form, personalizationNumberPrice: event.target.value })} placeholder="Ex. 5,00" className="mt-2 w-full rounded-xl border border-gray-300 bg-white px-4 py-3 font-normal focus:border-sbc focus:outline-none focus:ring-4 focus:ring-sbc/15" /><span className="mt-1 block text-xs font-normal text-gray-500">Saisissez 0 pour rendre cette option gratuite.</span></label>
                                        <p className="mt-4 text-xs font-black uppercase tracking-wide text-gray-600">Le numéro peut être placé</p><div className="mt-2 grid grid-cols-2 gap-2"><label className={`cursor-pointer rounded-xl border p-3 text-center text-sm font-bold ${form.personalizationNumberFrontEnabled ? "border-sbc bg-sbc text-white" : "border-gray-200 bg-white text-gray-600"}`}><input type="checkbox" checked={form.personalizationNumberFrontEnabled} onChange={(event) => setForm({ ...form, personalizationNumberFrontEnabled: event.target.checked })} className="sr-only" />Devant</label><label className={`cursor-pointer rounded-xl border p-3 text-center text-sm font-bold ${form.personalizationNumberBackEnabled ? "border-sbc bg-sbc text-white" : "border-gray-200 bg-white text-gray-600"}`}><input type="checkbox" checked={form.personalizationNumberBackEnabled} onChange={(event) => setForm({ ...form, personalizationNumberBackEnabled: event.target.checked })} className="sr-only" />Dos</label></div>
                                    </div> : <p className="mt-3 text-xs text-gray-500">Le numéro ne sera pas proposé.</p>}
                                </fieldset>
                            </div>
                        </section>}
                    </div>
