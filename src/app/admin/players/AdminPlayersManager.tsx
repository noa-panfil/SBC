...
type Person = { id: number; firstname: string; lastname: string; image_id: number | null; teams: string | null; roles: string | null; active: number };
    ...
    return <div className="space-y-5">
        ...
        <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
            {people.map((person: Person) => <Link key={person.id} href={`/admin/players/${person.id}`} className="flex items-center gap-4 border-b p-4 transition last:border-0 hover:bg-gray-50 md:p-5">
                <div className="h-12 w-12 shrink-0 overflow-hidden rounded-full bg-gray-100">{person.image_id ? <img src={`/api/image/${person.image_id}?scope=person`} alt="" className="h-full w-full object-cover" /> : <span className="flex h-full items-center justify-center text-gray-300"><i className="fas fa-user" /></span>}</div>
