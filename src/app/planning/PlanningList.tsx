...
function MatchCard({ match }: { match: Match }) {
    ...
    return (
        <div className={`group relative rounded-2xl p-6 transition-all duration-300 hover:transform hover:-translate-y-1 hover:shadow-xl overflow-hidden border ${isFeatured ? 'bg-green-50 border-sbc shadow-md z-10 scale-[1.02]' : 'bg-white hover:bg-gray-50 border-gray-200'}`}>
            {/* Background Decor */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                {/* Date Badge */}
                <div className={`flex flex-col items-center justify-center w-20 h-20 rounded-2xl border transition-colors shadow-sm shrink-0 ${isFeatured ? 'bg-sbc text-white border-sbc' : 'bg-white border-gray-200 group-hover:border-sbc'}`}>
                ...
                <div className="flex-1 text-center md:text-left">
                    ...
                    <h3 className="text-3xl font-black text-gray-800 italic leading-tight mb-2">
                        {match.category} <span className="text-sbc mx-2 text-xl font-black italic align-middle">VS</span> <span className="text-gray-500">{match.opponent}</span>
                    </h3>
                    <div className="flex items-center justify-center md:justify-start gap-4 text-sm text-gray-500 font-bold">
                        <div className="flex items-center gap-2">
                            <i className="fas fa-map-marker-alt text-gray-400 group-hover:text-sbc transition-colors"></i>
                            Salle Jesse Owens
                        </div>
                ...
                <div className="hidden md:block shrink-0 text-right">
                    <span className="text-5xl font-black text-gray-100 font-mono tracking-tighter group-hover:text-sbc/20 transition-colors">
