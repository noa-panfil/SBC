            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                                ...
                                {nextMatch ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center text-center h-full min-h-[300px]">
                                        <div className="mb-6 p-6 bg-white/5 rounded-full border border-white/10">
                                            <i className="fas fa-calendar-day text-6xl text-gray-600"></i>
                                        </div>
                                        <h3 className="text-3xl md:text-4xl font-black text-white italic uppercase mb-2">
                                            Pas de match à la une
                                        </h3>
                                        <p className="text-gray-400 font-medium text-lg max-w-md mx-auto">
                                            Aucun match majeur n'est programmé pour cette semaine. Consultez le calendrier complet pour plus de détails.
                                        </p>
                                    </div>
                                )}
                            </div>
                    ...
                    <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-4 h-full">
                        {[...upcomingMatches, ...Array(3 - upcomingMatches.length).fill(null)].map((match, idx) => (
                            <div key={idx} className={`group relative p-5 rounded-2xl transition duration-300 flex-1 flex flex-col justify-center ${match
                                ? "bg-gray-800/50 hover:bg-gray-800 border border-white/5 hover:border-sbc/30"
                                : "bg-gray-800/20 border border-white/5 border-dashed"
                                }`}>
                                {match ? (
                                    ...
                                    <div className="flex items-center justify-center h-full gap-3 opacity-50">
                                        <i className="fas fa-calendar-times text-2xl text-gray-500"></i>
                                        <span className="text-gray-400 font-bold uppercase text-sm tracking-wider">Pas de match programmé</span>
                                    </div>
                                )}
                            </div>
                        ))}
