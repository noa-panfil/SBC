...
type ImageRow = RowDataPacket & {
    id: number; product_id: number; image_id: number; is_primary: number; display_order: number;
};
type VariantRow = RowDataPacket & {
    id: number; product_id: number; sku: string | null; size: string; color: string; price_cents: number; is_active: number; display_order: number;
};
...
export async function getPublicProducts(): Promise<ShopProduct[]> {
    ...
    const [imageRows] = await pool.query<ImageRow[]>(
        `SELECT id, product_id, image_id, is_primary, display_order
         FROM shop_product_images WHERE product_id IN (${placeholders})
         ORDER BY is_primary DESC, display_order ASC, id ASC`,
        ids
    );
    const [variantRows] = await pool.query<VariantRow[]>(
        `SELECT id, product_id, sku, size, color, price_cents, is_active, display_order
